...
const YogaHighlights: React.FC = () => {
  return (
    <section className="bg-gradient-to-br from-white to-[#4377B2]/10 py-16 px-6 md:px-20">
      <h2 className="text-4xl md:text-5xl font-bold text-center text-[#4377B2] mb-14">
        Discover What Makes Our 50-Hour YTTC Special
      </h2>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        {/* Left: Ideal For */}
        <div className="bg-white shadow-lg rounded-xl p-8 border-l-4 border-[#4377B2]">
          <h3 className="text-2xl font-semibold text-[#4377B2] mb-6">Who Is This Course For?</h3>
          <ul className="space-y-4">
            {idealFor.map((point, idx) => (
              <li key={idx} className="flex items-start gap-3">
                <FaLeaf className="text-[#4377B2] mt-1" />
                <span className="text-[#4377B2]">{point}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="bg-white shadow-lg rounded-xl p-8 border-l-4 border-[#4377B2]">
          <h3 className="text-2xl font-semibold text-[#4377B2] mb-6">Training Highlights</h3>
          <ul className="space-y-4">
            {trainingHighlights.map((point, idx) => (
              <li key={idx} className="flex items-start gap-3">
                <FaStar className="text-[#4377B2] mt-1" />
                <span className="text-[#4377B2]">{point}</span>
              </li>
            ))}
          </ul>
