...
const teachers = [
  ...
  },
  {
    name: 'Purnima Ji',
    title: 'Lead teacher',
    image: '/Purnima ji.jpg', // Fixed filename typo
...
export default function TeachersSection21() {
  return (
    <section className="relative py-16 bg-transparent">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-8 md:px-16 text-center">
        <h2 className="text-3xl sm:text-4xl font-bold text-[#150e70] mb-4">
          Meet Your Instructors
        </h2>
        <p className="text-[#150e70] max-w-3xl mx-auto pb-8 text-sm sm:text-base">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {teachers.map((teacher, index) => (
            <div key={index} className="flex flex-col items-center text-center">
              <div className="w-32 h-32 sm:w-36 sm:h-36 md:w-40 md:h-40 mb-4 rounded-full overflow-hidden border-4 border-white shadow-lg">
                <Image
                  src={teacher.image}
                  alt={teacher.name}
                  width={160}
