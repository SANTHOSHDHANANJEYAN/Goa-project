...
export default function Schedule27Days() {
  const [isPlaying, setIsPlaying] = useState(false);
  const videoUrl = '/Videos/27daysvideo.mov';
          ...
          <ul className="space-y-3 text-sm sm:text-base flex-1">
            {schedule.map(([time, activity], idx) => (
              <li key={idx} className="flex items-start">
                <span className="font-semibold text-[#150e70] w-28 sm:w-32 flex-shrink-0">{time}</span>
              ...
              {isPlaying ? (
                ...
                <>
                  ...
                  <button
                    ...
                    className="absolute inset-0 flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-indigo-500"
